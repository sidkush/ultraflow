...
## Output Format (required — all 8 items, each on its own line)
```
## Plan Review
[BLOCKING] Coverage: PASS|FAIL — [reason]
[BLOCKING] Precision: PASS|FAIL — [reason]
[BLOCKING] File paths: PASS|FAIL — [reason]
[BLOCKING] Test commands: PASS|FAIL — [reason]
[BLOCKING] Dependencies: PASS|FAIL — [reason]
[BLOCKING] Naming: PASS|FAIL — [reason]
[ADVISORY] Scope: PASS|FAIL — [reason]
[ADVISORY] Commits: PASS|FAIL — [reason]

## Verdict: APPROVED | NEEDS REVISION
[If NEEDS REVISION: list exact [BLOCKING] items to fix]
## Confidence: [1-5]
```
**APPROVED requires all [BLOCKING] items to PASS.** [ADVISORY] FAILs are noted but not blocking. Keep response under 150 words.
