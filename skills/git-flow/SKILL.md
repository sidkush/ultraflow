...
- **Merge**: `git checkout main && git merge feature/<name> --no-ff`
  - If merge conflicts arise → STOP. List conflicting files. Do NOT auto-resolve. Ask user: "Merge conflict in: [files]. How should I resolve each?" Apply user guidance, then `git status` to confirm clean before committing.
- **Push + PR**: `git push -u origin feature/<name>` then `gh pr create`. Worktree remains active while PR is open — leave it running. Record path in `.worktrees/README.md` (create if needed) to prevent future orphaning.
- **Keep**: Do nothing. Record branch name and worktree path in `.worktrees/README.md` (create if needed) to prevent future orphaning.
